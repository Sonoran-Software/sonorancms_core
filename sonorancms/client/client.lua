			...
			for i = -1, GetVehicleMaxNumberOfPassengers(GetVehiclePedIsIn(ped)) + 1, 1 do
				...
				if (DoesEntityExist(pedPass)) then
					if (IsPedAPlayer(pedPass) and ped ~= pedPass) then
						...
						table.insert(vehicleData.passengers, {
							seat = i,
							passengerId = pedServerId
						})
					end
				end
			end
...
-- Death reasons
local fivemfivemDeathHashTable = {
	...
	[(GetHashKey('WAPAON_PIPEBOMB'))] = 'Pipe Bomb',
...
CreateThread(function()
	...
	while true do
		...
		if Config.EnableWeatherSync then
